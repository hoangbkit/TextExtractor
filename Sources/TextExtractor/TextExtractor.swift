...
public final class TextExtractor: Sendable {
    ...
    public var supportedFileExtensions: Set<String> { Set(extractors.flatMap { $0.supportedFileExtensions }) }
    ...
    public func extractor(forFileName fileName: String?, data: Data = Data()) -> (any TextFormatExtractor)? {
        if let ext = FileName.fileExtension(from: fileName),
           let match = extractors.first(where: { $0.supportedFileExtensions.contains(ext) }) {
            return match
        }
        return extractors.first { $0.canExtract(data: data, fileName: fileName) }
    }

    private func extract(data: Data, fileName: String?, sourceURL: URL?, options: TextExtractionOptions) throws -> ExtractedTextDocument {
